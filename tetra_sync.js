    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            [
                ...
                `1) **매칭 / 티켓 생성 (Match / Ticket)** — 구매자가 버튼을 누르면 ${rolePing}, 구매자, 판매자 3자만 보는 비공개 방이 생성됩니다.\n   → Buyer clicks the button and a **private 3-party room** is created (${rolePing}, buyer, seller).`,
                '2) **홀드 (Hold)** — 판매자가 먼저 관리자(에스크로 계정)에게 게임 내에서 키나를 보냅니다.\n   → **Seller sends all Kinah (including fee) in-game** to the escrow admin account first.',
                '3) **입금 (Pay)** — 관리자가 홀드 확인 후, 구매자가 실제 돈(USD/KRW 등)을 판매자에게 보냅니다.\n   → After admin confirms hold, **buyer sends real-world payment (USD/KRW, etc.)** to the seller.',
                '4) **완료 (Complete)** — 판매자가 입금 확인 후, 관리자가 키나를 구매자에게 전달하고, 봇이 Trust +1 을 부여한 뒤 티켓을 닫습니다.\n   → Once seller confirms payment, **admin delivers Kinah to buyer**, bot adds **Trust +1**, and the ticket is closed.',
                '',
